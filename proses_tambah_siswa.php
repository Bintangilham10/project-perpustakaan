...
if($_POST){
    ...
    elseif(empty($username)){
        ...
        values
        ('$nama_siswa','$tanggal_lahir','$gender','$alamat','$username','$password','$id_kelas')") or die(mysqli_error($conn));
        if($insert){
...
include "koneksi.php";
        $insert=mysqli_query($conn,"insert into siswa (nama_siswa,tanggal_lahir, gender, alamat, username, password, id_kelas)
        values
        ('".$nama_siswa."','".$tanggal_lahir."','".$gender."','".$alamat."','".
        $username."','".md5($password)."','".$id_kelas."')") or die(mysqli_error($conn));
        if($insert){
        echo "<script>alert('Sukses menambahkan siswa');location.href='tambah_siswa.php';</script>";
*/
?>